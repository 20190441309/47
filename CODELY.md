...
- [2026-09-05 11:00:27] 用户希望测试尽量由 AI 自动完成(2026-09-05「你不能直接帮我测试吗」):能自动化的(构建、HTTP 层验证、后端落盘数据核对、进程/产物轮询)都先做完并汇报结果,只把必须人工的浏览器/编辑器内点击交互留给用户,并说清楚为什么只能他们做。
...
- [2026-09-05 11:57:08] 团结引擎批处理坑(2026-09-05 实测):Tuanjie.exe 是 GUI 子系统程序,PowerShell 里 `& Tuanjie.exe -batchmode ...` 会立即返回、后台才真正跑构建——不能信命令返回/退出码,要靠轮询产物时间戳(如 unity/Builds/WebGLBaseline-SIZE.txt 的 LastWriteTime)或等编辑器进程退出来判断完成。同代码重跑走 Library/Bee 增量缓存,秒级完成(首次约 4-5 分钟)。跑批处理前必须先确认用户的 GUI 编辑器没开着本工程:工程被占用时批处理不弹错而是直接崩溃(日志留 stacktrace + Crash_ 报告目录);查进程要用 WMI Get-CimInstance Win32_Process 的 ExecutablePath(PS 的 Get-Process .Path 对部分进程取不到)。另:新增 .cs 文件后的**第一次**批处理可能用陈旧脚本清单编译(新文件 meta 已建但不在编译单元里,报不存在该类的 CS0103 假错),**再跑一次**即正常——批处理编译检查报「类不存在」先重跑一次再排查代码。
...
- [2026-09-05 11:00:27] 团结编辑器脚本编译失败时的隐蔽陷阱(2026-09-05 实测):菜单/工具在旧程序集上照常运行,症状是改动不生效、产物缺失、无任何弹窗。改完脚本让用户跑编辑器菜单前,先 grep %LOCALAPPDATA%\Tuanjie\Editor\Editor.log 确认没有 error CS(哪怕漏一个 using 都会 CS0246 拖一整轮返工)。另:场景文件 mtime 被用户编辑器打开时也会刷新,判断「场景是否重新生成」要看内容(如断链存根 m_ClassName)不能只看 mtime。
- [2026-09-05 11:57:08] 冒烟/测试跑法注意(2026-09-05 实测):`npm run smoke` 在 PowerShell 里会失败(npm 经 cmd 起的 sh 不认 `set -o pipefail`),smoke.sh 要在 WSL bash 里跑(`wsl -e bash -c "cd /mnt/e/hki/47/server && bash test/smoke.sh"`);但 PS 调 wsl 时输出可能是二进制乱码且 WSL 访问不到 Windows 回环——后端接口验证优先用 node 直跑脚本(server/test/m2-contract-check.js 模式,fetch 不经回环隔离问题),中文 JSON 一律 node 解析(PS 5.1 ConvertFrom-Json 遇 UTF-8 中文会炸)。
