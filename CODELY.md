...
- [2026-09-05 11:00:27] 团结引擎批处理坑(2026-09-05 实测):Tuanjie.exe 是 GUI 子系统程序,PowerShell 里 `& Tuanjie.exe -batchmode ...` 会立即返回、后台才真正跑构建——不能信命令返回/退出码,要靠轮询产物时间戳(如 unity/Builds/WebGLBaseline-SIZE.txt 的 LastWriteTime)或等编辑器进程退出来判断完成。同代码重跑走 Library/Bee 增量缓存,秒级完成(首次约 4-5 分钟)。跑批处理前必须先确认用户的 GUI 编辑器没开着本工程:工程被占用时批处理不弹错而是直接崩溃(日志留 stacktrace + Crash_ 报告目录);查进程要用 WMI Get-CimInstance Win32_Process 的 ExecutablePath(PS 的 Get-Process .Path 对部分进程取不到)。
