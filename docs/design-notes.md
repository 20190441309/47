...
- 2026-09-06:**M2 事件链路用户实测闭环**:①修「跳过对话直接修 bug」阶段卡死(事件表补 ch1_arrival→ch1_done,与对话路径殊途同归,离线镜像同步);②修**运行时 UI 接线不序列化的地雷**——场景生成器用 AddListener 接线不进场景文件,编辑器域重载(脚本重编译)即静默清空,曾致发送/快捷回复/重连/补丁台关闭按钮全部失灵;改为组件运行时自愈接线(QuickReplyRow/PatchBoard/DialogueManager 的 Awake 幂等重绑),构建器只注入序列化字段引用。**教训:程序化生成的场景,事件接线必须运行时自愈,不能依赖构建时的内存状态。**
